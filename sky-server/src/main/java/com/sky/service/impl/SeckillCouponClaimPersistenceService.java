...
public class SeckillCouponClaimPersistenceService {
    ...
    public void persist(SeckillCouponClaimMessage message) {
        // 流程4-步骤15（幂等检查）：按claimId查询领取记录；方法末尾再由同一Mapper完成insert。
        UserCoupon persistedCoupon = userCouponMapper.getByClaimId(message.getClaimId());
        if (persistedCoupon != null) {
            if (!message.getCouponId().equals(persistedCoupon.getCouponId()) || !message.getUserId().equals(persistedCoupon.getUserId())) {
                throw new IllegalStateException("领取流水ID对应的业务数据不一致，claimId=" + message.getClaimId());
            }
            log.info("秒杀券领取消息已落库，跳过重复消费，claimId={}", message.getClaimId());
            return;
        }

        // 流程4-步骤16：查询秒杀券，并在同一MySQL事务中扣减已经由Redis预扣的数据库库存。
        SeckillCoupon coupon = seckillCouponMapper.getById(message.getCouponId());
        if (coupon == null || coupon.getClaimEndTime() == null) {
            throw new IllegalStateException("秒杀券不存在或领取结束时间为空，claimId=" + message.getClaimId());
        }

        int affectedRows = seckillCouponMapper.decreaseStockAfterPreDeduct(message.getCouponId());
        if (affectedRows != 1) {
            throw new IllegalStateException("Redis已预扣但MySQL库存扣减失败，claimId=" + message.getClaimId());
        }
